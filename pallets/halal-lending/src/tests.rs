...
use crate::mock::*;
use crate::pallet::{Error, Event, LoanStatus, Loans, LoanRewards, NextLoanId, PriceProvider};
use polkadot_sdk::{
...
fn test_create_loan_flow() {
	new_test_ext().execute_with(|| {
		...
		println!("Pallet's balance: {}", Balances::free_balance(&HalalLending::account_id()));
...
fn test_add_rewards() {
	new_test_ext().execute_with(|| {
		...
		// Add rewards to the loan
		let reward_amount = 100;
		assert_ok!(HalalLending::add_rewards(
			RuntimeOrigin::root(),
			0, // loan_id
			reward_amount
		));
		...
		println!("\n=== REWARDS TEST ===");
		println!("✅ Rewards added successfully");
		println!("✅ Loan rewards tracking working");
	});
}
...
fn test_claim_rewards_on_repayment() {
	new_test_ext().execute_with(|| {
		...
		println!("Alice balance after repay: {}", Balances::free_balance(&ALICE));
		println!("Treasury balance after repay: {}", Balances::free_balance(&TREASURY));
		...
		assert_eq!(Balances::free_balance(&TREASURY), treasury_before_repay + platform_share);
